...
public class AutoScrollViewPager extends LoopViewPager {
    ...
    private void setViewPagerScroller() {
        try {
            ...
            scroller = new CustomDurationScroller(getContext(), new DecelerateInterpolator(3));//(Interpolator)interpolatorField.get(null));
...
//        if(true){
//        	// 由于使用了LoopViewpager 所有滚动下标不需要再次判断了
//        	setCurrentItem(currentItem+1, isBorderAnimation);
//        	return;
//        }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_MOVE && !isCanTouch) {
            return true;
        }

        if (action == MotionEvent.ACTION_UP && !isCanTouch) {
            touchX = ev.getX();
            touchY = ev.getY();
